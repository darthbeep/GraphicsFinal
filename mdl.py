...
def p_statement_light(p):
    """statement : LIGHT SYMBOL NUMBER NUMBER NUMBER NUMBER NUMBER NUMBER"""
    commands.append(p[1:])
...
#All 3 are the same and me added, it's just that they have different names
def p_statement_color(p):
	"""statement : COLOR NUMBER NUMBER NUMBER"""
	commands.append(tuple(p[1:]))

def p_statement_coutline(p):
	"""statement : COUTLINE NUMBER NUMBER NUMBER"""
	commands.append(tuple(p[1:]))
...
def p_statement_sphere(p):
    ...
    if len(p) == 6:
        commands.append((p[1], 'n', p[2], p[3], p[4], p[5], None))
    elif len(p) == 7:
        #print p[1]
        commands.append((p[1], 'y', p[3], p[4], p[5], p[6], p[2]))
    else:
        commands.append((p[1], 'n',p[2], p[3], p[4], p[5], [p[6], p[7]]))
...
def p_statement_torus(p):
    ...
    if len(p) == 7:
        commands.append((p[1],'n', p[2], p[3], p[4], p[5], p[6], None))
    elif len(p) == 8:
        commands.append((p[1], 'y',p[3], p[4], p[5], p[6], p[7], p[2]))
    else:
        commands.append((p[1],'n', p[2], p[3], p[4], p[5], p[6], [p[7], p[8]]))
...
def p_statement_box(p):
    ...
    if len(p) == 8:
        ...
        ret.append(p[1])
        ret.append('n')
        commands.append(tuple(ret+p[2:] +[None]))
		#print 'hi'
    else:
        #print 'hello'
        commands.append((p[1],'y', p[3], p[4], p[5], p[6], p[7], p[8], p[2]))
